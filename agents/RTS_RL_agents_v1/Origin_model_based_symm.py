"""Original model based model (single state) for the original two-step task.
"""
import numpy as np
...
@jit(nopython=True)
def _OMBs_step_core_variables(alpha1, alpha2, beta1, beta2, lbd, w,
                             p_transit,
                             c1, s2, c2, o,
                             Q_stage1_MF, Q_stage1_MB, Q_stage1_net, Q_stage2_state1, Q_stage2_state2,
                             ):
    ...
    if s2 == 0: # stage 2 state 1
        Q_stage1_MF_delta =  alpha1 * ((1 - lbd) * Q_stage2_state1[c2] + lbd * o)
        Q_stage1_MF_new[c1] = (1 - alpha1) * Q_stage1_MF[c1] + Q_stage1_MF_delta
        Q_stage1_MF_new[nc1] = (1 - alpha1) * Q_stage1_MF[nc1] - Q_stage1_MF_delta # anti-correlated update
        Q_stage2_state1_new[c2] = (1 - alpha2) * Q_stage2_state1[c2] + alpha2 * o
        Q_stage2_state1_new[nc2] = (1 - alpha2) * Q_stage2_state1[nc2] - alpha2 * o # anti-correlated update
    else:
        Q_stage1_MF_delta = alpha1 * ((1 - lbd) * Q_stage2_state2[c2] + lbd * o)
        Q_stage1_MF_new[c1] = (1 - alpha1) * Q_stage1_MF[c1] + Q_stage1_MF_delta
        Q_stage1_MF_new[nc1] = (1 - alpha1) * Q_stage1_MF[nc1] - Q_stage1_MF_delta # anti-correlated update
        Q_stage2_state2_new[c2] = (1 - alpha2) * Q_stage2_state2[c2] + alpha2 * o
        Q_stage2_state2_new[nc2] = (1 - alpha2) * Q_stage2_state2[nc2] - alpha2 * o # anti-correlated update
    ...
    Q_stage1_net_new = (1 - w) * Q_stage1_MF_new + w * Q_stage1_MB_new

    return Q_stage1_MF_new, Q_stage1_MB_new, Q_stage1_net_new, Q_stage2_state1_new, Q_stage2_state2_new


@jit(nopython=True)
def _OMBs_session_likelihood_core(alpha1, alpha2, beta1, beta2, lbd, w,
                             p_transit,
    ...
    for trial in range(n_trials):
        ...
        Q_stage1_MF[trial + 1], Q_stage1_MB[trial + 1], Q_stage1_net[trial + 1], Q_stage2_state1[trial + 1], Q_stage2_state2[trial + 1] = \
            _OMBs_step_core_variables(
            alpha1, alpha2, beta1, beta2, lbd, w,
            p_transit,
            c1, s2, c2, o,
            Q_stage1_MF[trial], Q_stage1_MB[trial], Q_stage1_net[trial], Q_stage2_state1[trial], Q_stage2_state2[trial],
        )
    ...
    return trial_log_likelihood, Q_stage1_MF, Q_stage1_MB, Q_stage1_net, Q_stage2_state1, Q_stage2_state2, \
...
class Origin_model_based_symm(TwoStepModelCoreOri):
    def __init__(self, p_transit=0.7, model_type='MF'):
        ...
        if model_type == 'MF':
            self.param_names =  ['alpha1', 'alpha2', 'beta1', 'beta2', 'lbd']
            self.w = 0
            self.params = [0.4, 0.4, 5., 5., 0.5]
            self.param_ranges = ['unit', 'unit', 'pos', 'pos', 'unit']
            self.n_params = 5
        elif model_type == 'MB':
            self.param_names = ['alpha2', 'beta1', 'beta2']
            self.alpha1 = 0
            self.lbd = 0
            self.w = 1
            self.params = [0.4, 5., 5.]
            ...
            self.param_names = ['alpha1', 'alpha2', 'beta1', 'beta2', 'lbd', 'w']
            self.params = [0.4, 0.4, 5., 5., 0.5,0.5]
            self.param_ranges = ['unit', 'unit', 'pos', 'pos', 'unit', 'unit']
            self.n_params = 6
    ...
    def _step_core_variables(self, trial_event, params):
        ...
        if self.model_type == 'MF':
            alpha1, alpha2, beta1, beta2, lbd = params
            w = self.w
        elif self.model_type == 'MB':
            alpha2, beta1, beta2 = params
            alpha1 = self.alpha1
            lbd = self.lbd
            w = self.w
        elif self.model_type == 'MX':
            alpha1, alpha2, beta1, beta2, lbd, w = params
        else:
            raise NotImplementedError
        self.wm['Q_stage1_MF'], self.wm['Q_stage1_MB'], self.wm['Q_stage1_net'], self.wm['Q_stage2_state1'], self.wm['Q_stage2_state2'] = _OMBs_step_core_variables(
            alpha1, alpha2, beta1, beta2, lbd, w,
            self.p_transit,
            c1, s2, c2, o,
            self.wm['Q_stage1_MF'], self.wm['Q_stage1_MB'], self.wm['Q_stage1_net'], self.wm['Q_stage2_state1'], self.wm['Q_stage2_state2']
        )
    ...
    def _session_likelihood_core(self, session, params, DVs):
        ...
        DVs['trial_log_likelihood'], DVs['Q_stage1_MF'], DVs['Q_stage1_MB'], DVs['Q_stage1_net'], DVs['Q_stage2_state1'], DVs['Q_stage2_state2'], \
            DVs['first_scores'], DVs['first_choice_probs'], DVs['second_state1_scores'], DVs['second_state1_choice_probs'], DVs['second_state2_scores'], DVs['second_state2_choice_probs'],DVs['trial_is_correct'] = _OMBs_session_likelihood_core(
            alpha1, alpha2, beta1, beta2, lbd, w,
            self.p_transit,
            session['first_choices'], session['second_states'], session['second_choices'], session['outcomes'], session['mask'],
            DVs['Q_stage1_MF'], DVs['Q_stage1_MB'], DVs['Q_stage1_net'], DVs['Q_stage2_state1'], DVs['Q_stage2_state2'],
