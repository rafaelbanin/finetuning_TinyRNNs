"""Model based model with reward offset. """
import numpy as np
...
@jit(nopython=True)
def _MB_step_core_variables(alpha, r_offset, p_transit, c, s, o, Q_s, Q_mb):
    nc = 1 - c  # Not chosen first step action.
    ns = 1 - s  # Not reached second step state.

    Q_s_new = Q_s.copy()
    # update action values.
    Q_s_new[s] = (1. - alpha) * Q_s[s] + alpha * (o + r_offset)
...
@jit(nopython=True)
def _MB_session_likelihood_core(alpha, r_offset, iTemp, p_transit, choices, second_steps, outcomes, Q_s, Q_mb, scores, choice_probs, n_trials):
    ...
    for trial in range(n_trials):
        ...
        Q_s[trial + 1], Q_mb[trial + 1] = _MB_step_core_variables(alpha, r_offset, p_transit, c, s, o, Q_s[trial], Q_mb[trial])
...
class Model_based_offset(TwoStepModelCoreCSO):
    def __init__(self, p_transit=0.8):
        super().__init__()
        self.name = 'Model based reward offset'
        self.param_names = ['alpha', 'r_offset', 'iTemp']
        self.params = [0.5, 0, 5.]
        self.param_ranges = ['unit', 'unc', 'pos']
        self.n_params = 3
    ...
    def _step_core_variables(self, trial_event, params):
        (c, s, o) = trial_event
        alpha, r_offset, iTemp = params
        self.wm['Q_s'], self.wm['Q_mb'] = _MB_step_core_variables(alpha, r_offset, self.p_transit, c, s, o, self.wm['Q_s'], self.wm['Q_mb'])
    ...
    def _session_likelihood_core(self, session, params, DVs):
        ...
        DVs['trial_log_likelihood'], DVs['Q_s'], DVs['Q_mb'], DVs['scores'], DVs['choice_probs'] = _MB_session_likelihood_core(
            alpha, r_offset, iTemp, self.p_transit, session['choices'], session['second_steps'], session['outcomes'],
