...
class MF(RL_agent):
    ...
    def session_likelihood(self, session, params, get_DVs = False):
        ...
        # Unpack parameters.
        alp, iTemp, lbd = params[:3]   # Q value decay parameter.
        ...
        Q_net = self.apply_kernels(Q, choices, second_steps, params)

        if get_DVs: return self.get_DVs(session, params, Q_net, Q) | {'session_log_likelihood': session_log_likelihood(choices, Q_net, iTemp), 'scores': Q_net * iTemp}
        else:       return session_log_likelihood(choices, Q_net, iTemp)


    def simulate(self, task, n_trials, params=None, get_DVs=True):
        if params is None:
            params = self.params

        alp, iTemp, lbd = params[:3] 
        ...
        DVs = {'Q': Q, 'V': V, 'n_trials': n_trials, 'choices': choices, 'second_steps': second_steps, 'outcomes': outcomes}
        return DVs
