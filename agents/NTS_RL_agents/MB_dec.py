...
class MB_dec(RL_agent):
    ...
    def session_likelihood(self, session, params, get_DVs = False):
        ...
        # Unpack parameters.
        alpV, decV, alpT, decT, iTemp = params[:5]   
        ...
        Q_net = self.apply_kernels(M, choices, second_steps, params)

        if get_DVs: return self.get_DVs(session, params, Q_net, None, M) | {'session_log_likelihood': session_log_likelihood(choices, Q_net, iTemp), 'scores': Q_net * iTemp}
    ...
    def simulate(self, task, n_trials, params=None, get_DVs=True):
        ...
        DVs = {'T': T, 'V': V, 'n_trials': n_trials, 'choices': choices, 'second_steps': second_steps,
               'outcomes': outcomes}
        return DVs